...
void Removerter::makeGlobalMap( void )
{
    ...
    if( kFlagSaveMapPointcloud ) {
        // in global coord
        std::string origin_global_file_name = save_pcd_directory_ + "OriginalNoisyMapGlobal.pcd";
        pcl::io::savePCDFileBinary(origin_global_file_name, *map_global_curr_);
        ROS_INFO_STREAM("\033[1;32m The original pointcloud is saved (global coord): " << origin_global_file_name << "\033[0m");   
...
void Removerter::saveCurrentStaticAndDynamicPointCloudGlobal( void )
{
    ...
    pcl::io::savePCDFileBinary(static_file_name, *map_global_static);
...
void Removerter::removeOnce( float _res_alpha )
{
    ...
    *map_global_static += *map_global_curr_static_;
